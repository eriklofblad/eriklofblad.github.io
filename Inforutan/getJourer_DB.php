...
$jour_file = "cache/cache-jourer_db.json";
...
function getMedinetInfo($jourkoder, $jour_file){
    ...
    foreach ($res as $n => $responsearray){
        foreach($responsearray as $i => $response){
            ...
            $finalJSON[]= array("site"=>$site[$n], "startdate"=>$startdate, "jourtyp"=>utf8_encode($jourtyp), "jourtod"=>$jourtod,"starttime"=>$starttime, "stopptime"=>$stopptime, "journamn"=>utf8_encode($journamn));
        }
    }
    if($debug){
